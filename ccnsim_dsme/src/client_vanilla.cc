...
#include "ccnsim.h"
#include "client_vanilla.h"

#include "error_handling.h"
#include <random>

Register_Class (client_vanilla);


void client_vanilla::initialize()
{
	...
	if (find(content_distribution::clients, content_distribution::clients + num_clients ,getNodeIndex()) != content_distribution::clients + num_clients)
	{
		...
		if(!onlyModel)		// The scheduling of clients requests should be done.
		{
			...
			if(down > 1)			// TTL-based scenario (ModelGraft)
			{
				...
				scheduleAt( simTime() + getAncestorPar("client_warmup") + uniform(0,(double)(1./(double)(lambda/down))), arrival_ttl);
			}
			else if(down == 1)		// ED-sim scenario: schedule one request with the overall lambda mean.
			{
				arrival = new cMessage("arrival", ARRIVAL);
                if (getAncestorPar("client_deterministic")) {
                    scheduleAt( simTime() + getAncestorPar("client_warmup"), arrival);
                }
                else {
                    scheduleAt( simTime() + getAncestorPar("client_warmup") + uniform(0,1./lambda), arrival);
                }
			}
			else
			{
...
void client_vanilla::finish()
{
	client::finish();
}

void client_vanilla::handle_timers(cMessage *timer)
{
...
void client_vanilla::handleMessage(cMessage *in)
{
    ...
    if (in->isSelfMessage())	// A self-generated message can be either an 'ARRIVAL', an 'ARRIVAL_TTL', or a 'TIMER'.
    {
		...
		{
		case ARRIVAL:
			request_file(chunk_counter++);   // 'newCard+1' is needed to discern between ED and ModelGraft.
            if (chunk_counter < content_distribution::catalog.size()) {
                if (getAncestorPar("client_deterministic")) {
                    scheduleAt( simTime() + 1./lambda, arrival );
                }
                else {
 ...
 */
void client_vanilla::request_file(unsigned long nameC)
{
	name_t name;
